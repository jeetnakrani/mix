...
function main(data, renderFilter)
{
    ...
    if (!data || data.length === 0)
    {
        ...
        document.title = "Vaccinate ALL | " + currentDistrict
...
function start()
{
    ...
    document.getElementById('watchHeading').innerHTML =
        "<div class='watchingText'> <div id='watchingDot'></div><span>Vaccinate ALL is now monitoring Co-WIN portal every " +
        Math.round(refreshInterval / 1000) +
        " seconds.</span></div> <div style='text-align:center;'>You'll be notified if Vaccinate ALL finds new vaccination slots in " +
...
function playAlert(newCentres)
{
    ...
    if (browserNotificationCheckbox.checked)
        new Notification('Vaccinate ALL Alert', { body: createSentence(newCentres, true), icon: relativePath + "./assets/img/icon.png" })
}
